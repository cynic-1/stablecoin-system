...
def print_summary(results, group_keys):
    ...
    print(f"{header} {'Con.TPS':>10} {'Con.Lat':>10} {'E2E TPS':>10} {'E2E Lat':>10}")
    print('-' * (20 * len(group_keys) + 44))
    for key, runs in sorted(grouped.items()):
        avg = lambda field: sum(r[field] for r in runs) / len(runs)
        row = ''.join(f'{str(v):<20}' for v in key)
        print(f"{row} {avg('consensus_tps'):>10,.0f} {avg('consensus_latency_ms'):>10,.0f}"
              f" {avg('e2e_tps'):>10,.0f} {avg('e2e_latency_ms'):>10,.0f}")
