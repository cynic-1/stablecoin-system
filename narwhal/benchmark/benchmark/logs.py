...
class LogParser:
    ...
    def _parse_primaries(self, log):
        ...
        # Parse execution timestamps (e2e_exec feature).
        tmp = findall(r'\[(.*Z) .* Executed B\d+\(\d+\) -> ([^ ]+) in', log)
        tmp = [(d, self._to_posix(t)) for t, d in tmp]
    ...
    def _with_exec_throughput(self):
        ...
        if duration <= 0:
            return 0, 0, 0
        bytes = sum(
            self.sizes[d] for d in self.executions if d in self.sizes
        )
        bps = bytes / duration
        tps = bps / self.size[0]
        return tps, bps, duration

    def _with_exec_latency(self):
        latency = [
            self.executions[d] - self.proposals[d]
